...
OUT    = f"{BASE}/revision/figures"          # R1-5: write to revision/, do NOT touch results/
os.makedirs(OUT, exist_ok=True)

# R1-5: enlarge fonts globally for print legibility
...
# data1 → HCC, data3 → Multi (paper 표기)
METHODS = [
    {"label": "PBertKla (HCC)",          "short": "PBertKla\nHCC",
     "ds": "data1", "kind": "dl",    "color": "#9DAEC8"},
    {"label": "PBertKla (Multi)",        "short": "PBertKla\nMulti",
     "ds": "data3", "kind": "dl",    "color": "#3F7C9E"},
    {"label": "PBertKla-stack (HCC)",    "short": "PBertKla-stack\nHCC",
     "ds": "data1", "kind": "stack", "color": "#E1A56F"},
    {"label": "PBertKla-stack (Multi)",  "short": "PBertKla-stack\nMulti",
...
def make_figure_A():
    ...
    for ax, (key, mname) in zip(axes, metrics_keys):
        ...
        ax.set_ylim(ylo, yhi)
        ax.set_xticks(x)
        ax.set_xticklabels(x_short, fontsize=13)
        ax.set_xlim(-0.55, n_method - 0.45)

        ax.set_title(mname, fontsize=18, fontweight="bold", pad=10)
    ...
    fig.legend(handles=handles, loc="lower center", bbox_to_anchor=(0.5, -0.04),
               ncol=5, fontsize=12.5, framealpha=0.95, edgecolor="black")
...
def make_figure_B():
    ...
    for ax, r in zip(axes, results):
        ...
        # 제목 — 방법 이름 (2줄)
        ax.set_title(r["short"], fontsize=15, fontweight="bold", pad=8)
        ax.set_aspect("equal")
    ...
    fig.text(0.5, 0.02,
             ...
             f"·   Total samples: HCC n={n_d1}, Multi n={n_d2}",
...
def make_figure_C():
    ...
    ax.set_title("ROC Curve", fontsize=17, fontweight="bold", pad=10)
...
def make_figure_D():
    ...
    ax.set_title("PRC Curve", fontsize=17, fontweight="bold", pad=10)
    ax.tick_params(axis="both", labelsize=13.5)
